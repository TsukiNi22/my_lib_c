...
static int get_info(printf_data_t *data)
{
    ...
    if (my_malloc_c(&data->info, data->end + 1) == KO)
        return err_prog(UNDEF_ERR, "In: get_info 2", KO);
    if (!my_strncpy(data->info, &(data->str[data->i + 1]), data->end))
        return err_prog(UNDEF_ERR, "In: get_info 3", KO);
    return OK;
}
...
{
    if (!data || !identifier)
        return err_prog(PTR_ERR, "In: malloc_info", KO);
    *identifier = true;
    if (my_malloc_c(&(data->info_flags), len) == KO)
        return err_prog(UNDEF_ERR, "In: malloc_info 1", KO);
    if (my_malloc_c(&(data->info_field), len) == KO)
        return err_prog(UNDEF_ERR, "In: malloc_info 2", KO);
    if (my_malloc_c(&(data->info_precision), len) == KO)
        return err_prog(UNDEF_ERR, "In: malloc_info 3", KO);
    if (my_malloc_c(&(data->info_modifier), len) == KO)
        return err_prog(UNDEF_ERR, "In: malloc_info 4", KO);
    return OK;
}
...
static int extract_flags(printf_data_t *data, int *len, bool *identifier)
{
    ...
    if (!data || !len || !identifier)
        return err_prog(PTR_ERR, "In: extract_flags", KO);
    for (int i = 0; i <= *len; i++) {
        res = my_get_index(FLAGS, data->info[*len - i]);
        if (res == -2)
            return err_prog(UNDEF_ERR, "In: extract_flags", KO);
        if (res < 0) {
            *identifier = false;
            return OK;
        }
    ...
    if (!my_strncpy(data->info_flags, data->info, *len + 1))
        return err_prog(UNDEF_ERR, "In: extract_modifier", KO);
    return OK;
}
...
static int extract_field(printf_data_t *data, int *len, bool *identifier)
{
    ...
    if (!data || !len || !identifier)
        return err_prog(PTR_ERR, "In: extract_field", KO);
    for (i = 0; i <= *len; i++) {
        res = my_get_index(FIELD_WIDHT, data->info[*len - i]);
        if (res == -2)
            return err_prog(UNDEF_ERR, "In: extract_field", KO);
        if (res < 0)
            break;
    }
...
static int extract_precision(printf_data_t *data, int *len, bool *identifier)
{
    ...
    if (!data || !len || !identifier)
        return err_prog(PTR_ERR, "In: extract_precision", KO);
    ...
    if (!my_strncpy(data->info_precision, &(data->info[index + 1]), i))
        return err_prog(UNDEF_ERR, "In: extract_precision", KO);
    *len -= i + 1;
    return OK;
}
...
static int extract_modifier(printf_data_t *data, int *len, bool *identifier)
{
    ...
    if (!data || !len || !identifier)
        return err_prog(PTR_ERR, "In: extract_modifier", KO);
    for (i = 1; i <= *len; i++) {
        res = false;
...
static int extract_info(printf_data_t *data, bool *identifier)
{
    ...
    if (!data || !identifier)
        return err_prog(PTR_ERR, "In: extract_info", KO);
    len = my_strlen(data->info) - 1;
    if (len < 0)
        return err_prog(UNDEF_ERR, "In: extract_info 1", KO);
    data->info_specifiers = data->info[len];
    if (malloc_info(data, len + 2, identifier) == KO)
        return err_prog(UNDEF_ERR, "In: extract_info 2", KO);
    if (extract_modifier(data, &len, identifier) == KO)
        return err_prog(UNDEF_ERR, "In: extract_info 3", KO);
    if (identifier && extract_precision(data, &len, identifier) == KO)
        return err_prog(UNDEF_ERR, "In: extract_info 4", KO);
    if (identifier && extract_field(data, &len, identifier) == KO)
        return err_prog(UNDEF_ERR, "In: extract_info 5", KO);
    if (identifier && extract_flags(data, &len, identifier) == KO)
        return err_prog(UNDEF_ERR, "In: extract_info 6", KO);
    return OK;
}
...
{
    if (!data)
        return err_prog(PTR_ERR, "In: free_info", KO);
    if (data->info)
        free(data->info);
    if (data->info_flags)
...
{
    if (!data || !identifier)
        return err_prog(PTR_ERR, "In: identifier", KO);
    ...
    if (get_info(data) == KO)
        return err_prog(UNDEF_ERR, "In: identifier 1", KO);
    if (!data->info)
        return OK;
    if (extract_info(data, identifier) == KO || check(data, identifier) == KO)
        return err_prog(UNDEF_ERR, "In: identifier 2", KO);
    if (!(*identifier))
        return free_info(data);
    if (specifiers_dispatch(data) == KO)
        return err_prog(UNDEF_ERR, "In: identifier 3", KO);
    data->i += data->end;
    return free_info(data);
}
