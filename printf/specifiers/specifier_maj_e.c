...
static int handle_zero_precision(char *str, char *my_double, int precision)
{
    if (precision != 0)
        return 0;
    if (str[0] == '!') {
...
static int set_str(char *str, double nbr, long double e, int precision)
{
    ...
    handle_zero_precision(str, my_double, precision);
    size = my_strlen(str);
...
int specifier_maj_e(printf_data_t *data)
{
