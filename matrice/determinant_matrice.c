...
float determinant_matrice_2(matrice_t *matrice)
{
    ...
    if (!matrice)
        return err_prog(PTR_ERR, "In: determinant_matrice_2", 0);
    if (matrice->x != 2 || matrice->y != 2)
        return err_prog(ARGV_ERR, "In: determinant_matrice_2", 0);
...
float determinant_matrice(matrice_t *matrice)
{
    ...
    if (!matrice)
        return err_prog(PTR_ERR, "In: determinant_matrice", 0);
    if (matrice->x != matrice->y)
        return err_prog(ARGV_ERR, "In: determinant_matrice", 0);
    if (matrice->x == 1 && matrice->y == 1)
        return matrice->matrice[0][0];
    for (int i = 0; i < matrice->y; i++) {
        matrice_reduce = reduce_matrice(matrice, 0, i);
        if (!matrice_reduce)
            return err_prog(UNDEF_ERR, "In: determinant_matrice", 0);
